...
const UserQuota = () => {
  ...
  const percentage = {
    documents: Math.round((data?.Documents / QUOTA) * 100),
    photos: Math.round((data?.Photos / QUOTA) * 100),
  };
        ...
        <>
          ...
          <div className="pt-2">
            <div className="flex h-2 w-full overflow-hidden rounded-full bg-slate-200">
              {/* Documents segment */}
              <div
                className={clsx("bg-red-500", `w-[${percentage.documents}%]`)}
              />
              {/* Photos segment */}
              <div
                className={clsx("bg-blue-500", `w-[${percentage.photos}%]`)}
              />
            </div>
          </div>
        </>
